...
class RegisterFormTest extends TestCase
{
    ...
    public function register_user_true()
    {
        ...
        $response->assertRedirect('bookshelf'); // リダイレクト確認
    }
    ...
    public function register_name_max_alphabet_False()
    {
        ...
        $error = session('errors')->first('name');
        $this->assertEquals('名前は、10文字以下で指定してください。', $error);
    }
    /**
     * @test
     * [正常値] 最大値チェック（日本語）
    ...
    public function register_password_min_false()
    {
        ...
        $error = session('errors')->first('password');
        $this->assertEquals('パスワードは、6文字以上で指定してください。', $error);
    }
    /**
     * @test
     * [正常値] パスワード / 文字数チェック（最小値）& パスワード再入力確認
    ...
    {
        $response = $this->post('register', [
            'password' => str_repeat('a', 6), // a
            'password_confirmation' => str_repeat('b', 6) // b
        ]);
    ...
    {
        $response = $this->post('register', [
            'password' => str_repeat('あ', 6),
            'password_confirmation' => str_repeat('あ', 6)
        ]);
    ...
    public function register_password_invalid_signs_false()
    {
        ...
        foreach ($signs as $sign) {
            $response = $this->post('register', [
                'password' => str_repeat($sign, 6),
                'password_confirmation' => str_repeat($sign, 6)
            ]);
